...
-(void)postrequest_methodData{
    ...
    NPrintLog(@"ordercode%@",self.ordercode);
...
-(void)bujuSrollView
{
    ...
    if (self.images.count>0) {
        ...
        CGFloat hei = 30;
        if (wentArray.count == 6) {
            hei = 60;
            CGRect newFrame = CGRectMake(view2.frame.origin.x, view2.frame.origin.y, view2.frame.size.width, view2.frame.size.height);
            view2.frame = CGRectMake(newFrame.origin.x,newFrame.origin.y , newFrame.size.width, newFrame.size.height+30);
            imagesScrollView.frame = CGRectMake(0, 60, kWindowW, 100);
            gunHeight += 30;
        }
        
        UIView *view2View = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kWindowW, hei)];
        ...
        for (int i = 0; i<wentArray.count; i++) {
            ...
            if (i == 5) {
                la.frame = CGRectMake(5, 33, 60, 25);
            }else{
                la.frame = CGRectMake(65*i+5, 3, 60, 25);
            }
...
//    随车装备
    ...
    if (newGoods.count >0) {
        ...
        UIView *view4 = [[UIView alloc]initWithFrame:CGRectMake(0, gunHeight-10, kWindowW, (newGoods.count/4+1)*30+10)];
        view4.backgroundColor = [UIColor whiteColor];
        gunHeight +=  ((newGoods.count/4+1)*30+10);
        ...
        for (int i = 0; i<newGoods.count; i++) {
            ...
            la.font = [UIFont systemFontOfSize:13];
            la.backgroundColor = kZhuTiColor;
...
-(void)imagesButtonChick:(UIButton *)sender
{
    ...
    NSMutableArray *chuanArray = [[NSMutableArray alloc]init];
    for (int i = 0; i<self.images.count; i++) {
        NSDictionary *images = self.images[i];
        ...
        [chuanArray addObject:model];
    }
    vc.tuPianArray = chuanArray;
    vc.index = index;

...
-(void)dangQianBuJU
{
    ...
    for (int i = 0; i<self.colorArray.count; i++) {
        ...
        [coLabelbt  setBackgroundImage:[UIImage imageWithUIColor:kNavBarColor] forState:(UIControlStateSelected)];
...
-(void)okButtonChick:(UIButton *)sender
{
    ...
    if (zhuCeTimeLabel.text.length <= 0) {
        [self showMessageWithContent:@"请选择注册日期" point:self.view.center afterDelay:2.0];
        return;
    }
    self.chuanZhiDict.register_date = zhuCeTimeLabel.text;
    
    if (faZhengTimeLabel.text.length <= 0) {
        [self showMessageWithContent:@"请选择发证日期" point:self.view.center afterDelay:2.0];
        return;
    }

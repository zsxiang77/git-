...
-(instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style withTuoDong:(BOOL)tuoDong
{
...
- (void)cellSelectedAtIndexPath:(NSIndexPath *)indexPath{
    TheWorkbenchCell *cell = [self cellForRowAtIndexPath:indexPath];
    ...
    /**选中的cell准备好可以移动的时候*/
    if ([self.delegate respondsToSelector:@selector(cellBecomeMovingInTableView:)]) {
        [self.delegate cellBecomeMovingInTableView:self];
    }
}
/**
 ...
 */
- (void)didEndDraging{
    TheWorkbenchCell *cell = [self cellForRowAtIndexPath:_originalIndexPath];
    cell.hidden = NO;
    cell.alpha = 0;
    [UIView animateWithDuration:0.2 animations:^{

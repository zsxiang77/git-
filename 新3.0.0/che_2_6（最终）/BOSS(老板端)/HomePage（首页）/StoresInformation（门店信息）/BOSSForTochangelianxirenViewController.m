//
//  BOSSForTochangelianxirenViewController.m
//  cheDianZhang
//
...
@interface BOSSForTochangelianxirenViewController ()
<UITextFieldDelegate>
@property(nonatomic,strong)UITextField*mainTextFile;
@end

@implementation BOSSForTochangelianxirenViewController
...
-(void)buttonRight:(UIButton*)seader
{
    if(self.mainTextFile.text.length<=0){
        [self showMessageWithContent:@"请输入联系人" point:self.view.center afterDelay:1];
        return;
    }
    kWeakSelf(weakSelf)
    NSMutableDictionary*mDict=[NSMutableDictionary dictionaryWithCapacity:10];
    [mDict setObject:self.mainTextFile.text forKey:@"contact"];

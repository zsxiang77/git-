...
@interface SaoMiaoSFZViewController ()<UIAlertViewDelegate>{
...
@implementation SaoMiaoSFZViewController
-(void)dealloc{
...
- (void)getRecogResult{
    ...
    if (![allResult isEqualToString:@""]) {
        NSString *store_alias = [self.cardRecog GetRecogResultWithIndex:1];
        NSString *id_card = [self.cardRecog GetRecogResultWithIndex:6];
        
        NSString *address = [self.cardRecog GetRecogResultWithIndex:5];
        NSString *birthday = [self.cardRecog GetRecogResultWithIndex:4];
        NSString *nation = [self.cardRecog GetRecogResultWithIndex:3];
        NSString *sex = [self.cardRecog GetRecogResultWithIndex:2];
        
        self.chuanSanGeZhiBlock(store_alias, id_card,address,birthday,nation,sex);
        
        [self dismissViewControllerAnimated:YES completion:^{
            
        }];
//        [self shangChuanTuPian:_cropImagepath];
...
-(void)shangChuanTuPian:(NSString *)moderl
{
    UIImage *image = [UIImage imageWithContentsOfFile:moderl];

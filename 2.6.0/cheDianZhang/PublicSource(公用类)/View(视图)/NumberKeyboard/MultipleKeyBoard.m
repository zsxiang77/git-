...
@implementation MultipleKeyBoard
...
- (void)buildMainView:(NSString*)currentValue
{
    ...
    UILabel* touLabel = [CommonControlOrView setLabelWithFrame:CGRectMake(CGRectGetWidth(self.contentView.frame)-120, 0, 30, 45) textColor:kRGBColor(168, 168, 168) font:DJBoldSystemFont(13) text:@"投" textAlignment:NSTextAlignmentCenter];
    ...
    UILabel* beiLabel = [CommonControlOrView setLabelWithFrame:CGRectMake(CGRectGetMaxX(m_valueButton.frame), 0, 30, 45) textColor:kRGBColor(168, 168, 168) font:DJBoldSystemFont(13) text:@"倍" textAlignment:NSTextAlignmentCenter];
    [topView addSubview:beiLabel];
    
    /////
    float btnW = CGRectGetWidth(self.contentView.frame)/5.0;
    NSArray* multiples = @[@"1倍", @"2倍", @"5倍", @"10倍", @"20倍"];
    for (int i = 0; i < 5; i++) {
    ...
    NSArray* nums = @[@"1", @"2", @"3", @"4", @"5", @"6", @"7", @"8", @"9", @"0", @"+1", @"-1"];
    for (int i = 0; i < 12; i++) {
    ...
    [okBtn setBackgroundColor:kNavBarColor];
...
- (void)numButtonClick:(UIButton*)tempBtn
{
    ...
    {
        if (currentText.length > 0 && [currentText integerValue] > 1){
           currentText = [NSString stringWithFormat:@"%ld", (long)[currentText intValue]-1];
        }
    }

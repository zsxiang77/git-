...
-(void)refelesePeiJianWithModel:(OrderDetailPartsModel *)model
{
    ...
    self.nameLabel.text = model.parts_name;
    if (model.count.length<=0) {
        self.kuCunLabel.text = @"库存：0.00";
    }else{
        self.kuCunLabel.text = [NSString stringWithFormat:@"库存：%@",model.count];
    }
    
    self.bianHaoLabel.text = [NSString stringWithFormat:@"编号：%@",model.parts_code];
}

...
- (void) initRecog
{
    ...
    int intRecog = [self.cardRecog InitIDCardWithDevcode:@"6L2M5PEP572RKOW" recogLanguage:initLanguages];
...
- (void)getRecogResult{
    ...
    if (![allResult isEqualToString:@""]) {
        XinShiZheng_carsModel *chuanZhiDict = [[XinShiZheng_carsModel alloc]init];
        chuanZhiDict.images = _cropImagepath;
        chuanZhiDict.imagesLocal = _cropImagepath;
        chuanZhiDict.owner = [self.cardRecog GetRecogResultWithIndex:3];
        chuanZhiDict.address = [self.cardRecog GetRecogResultWithIndex:4];
        chuanZhiDict.use_character = [self.cardRecog GetRecogResultWithIndex:10];
        chuanZhiDict.model = [self.cardRecog GetRecogResultWithIndex:5];
        chuanZhiDict.issue_date = [self.cardRecog GetRecogResultWithIndex:9];
        chuanZhiDict.register_date = [self.cardRecog GetRecogResultWithIndex:8];
        chuanZhiDict.carvin = [self.cardRecog GetRecogResultWithIndex:6];
        chuanZhiDict.engine_number = [self.cardRecog GetRecogResultWithIndex:7];
        chuanZhiDict.carno = [self.cardRecog GetRecogResultWithIndex:1];
        
        [self shangChuanTuPian:chuanZhiDict];
...
-(void)shangChuanTuPian:(XinShiZheng_carsModel *)moderl
{
    UIImage *image = [UIImage imageWithContentsOfFile:moderl.images];
    NSMutableArray *arrar = [[NSMutableArray alloc]init];
    [arrar addObject:image];
    
    kWeakSelf(weakSelf)
    [self showOrHideLoadView:YES];
    [NetWorkManager requestDuoZhangWithParametersUIImageJPEGRepresentationWithUrl:@"order/order_queue/upload_file" viewController:nil isShowLoading:YES withimage:arrar success:^(id responseObject) {
        NSDictionary *adData = kParseData(responseObject);
        if (![adData isKindOfClass:[NSDictionary class]]) {
            return ;
        }
        moderl.images = KISDictionaryHaveKey(adData, @"names");
        
        [self showOrHideLoadView:NO];
        [weakSelf dismissViewControllerAnimated:YES completion:^{
            weakSelf.saoMiaoXSZHuiBlcok(moderl);
        }];

...
@property (nonatomic, strong) NSArray *titleLbTapArr;
@end
...
-(void)setUpUI{
    ...
    NSMutableArray *titleLbTaps = [NSMutableArray array];
    for (int i = 0;i<self.titles.count; i++) {
        ...
        [titleLbTaps addObject:tap];
    }
    self.titleLbTapArr = titleLbTaps.copy;
...
-(void)selectedLabel:(UITapGestureRecognizer *)sender{
    ...
    if ([self respondsToSelector:@selector(selectedLabel:)]) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(dw_segmentedControl:didSeletRow:)]) {
...
{
    if (row >= self.titleLbTapArr.count) {
        return;
    }
    
    UITapGestureRecognizer *tap = [self.titleLbTapArr objectAtIndex:row];
    [self selectedLabel:tap];
}

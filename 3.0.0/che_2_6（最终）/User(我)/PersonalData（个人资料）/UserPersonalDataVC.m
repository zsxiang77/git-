...
@property(nonatomic,strong)UILabel *jueSeQieHuanYouLabel;
...
//    [guanYuBt setTitle:@"关于我们" forState:(UIControlStateNormal)];
    ...
    UIView *jueSeQieHuanView = [[UIView alloc]initWithFrame:CGRectMake(0, jiSuanHei, kWindowW, 94/2)];
    jueSeQieHuanView.backgroundColor = [UIColor whiteColor];
    [mainScrollView addSubview:jueSeQieHuanView];
    jiSuanHei += 94/2+10;
    
    UILabel *jueSeQieHuanLabel = [[UILabel alloc]init];
    jueSeQieHuanLabel.font = [UIFont systemFontOfSize:14];
    jueSeQieHuanLabel.text = @"角色切换";
    jueSeQieHuanLabel.textColor = kRGBColor(102, 102, 102);
    [jueSeQieHuanView addSubview:jueSeQieHuanLabel];
    [jueSeQieHuanLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(10);
        make.top.bottom.mas_equalTo(0);
    }];
    
    UIImageView *xiaoXiImageView2 = [[UIImageView alloc]initWithImage:DJImageNamed(@"back_btn-1")];
    [jueSeQieHuanView addSubview:xiaoXiImageView2];
    [xiaoXiImageView2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(-10);
        make.centerY.mas_equalTo(jueSeQieHuanView);
        make.width.mas_equalTo(19/2);
        make.height.mas_equalTo(((19/2)*81)/48);
    }];
    
    self.jueSeQieHuanYouLabel = [[UILabel alloc]init];
    self.jueSeQieHuanYouLabel.textAlignment = NSTextAlignmentCenter;
    [self.jueSeQieHuanYouLabel.layer setMasksToBounds:YES];
    [self.jueSeQieHuanYouLabel.layer setCornerRadius:5];
    self.jueSeQieHuanYouLabel.backgroundColor = kRGBColor(83, 179, 62);
    self.jueSeQieHuanYouLabel.font = [UIFont systemFontOfSize:15];
    self.jueSeQieHuanYouLabel.textColor = [UIColor whiteColor];
    [jueSeQieHuanView addSubview:self.jueSeQieHuanYouLabel];
    [self.jueSeQieHuanYouLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(-30);
    ...
    UIButton *jueSeQieHuanBt = [[UIButton alloc]init];
    jueSeQieHuanBt.titleLabel.font = [UIFont systemFontOfSize:14];
    //    [guanYuBt setTitle:@"关于我们" forState:(UIControlStateNormal)];
    [jueSeQieHuanBt setTitleColor:[UIColor blackColor] forState:(UIControlStateNormal)];
    [jueSeQieHuanBt addTarget:self action:@selector(jueSeQieHuanBtChick:) forControlEvents:(UIControlEventTouchUpInside)];
    [jueSeQieHuanView addSubview:jueSeQieHuanBt];
    [jueSeQieHuanBt mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.mas_equalTo(0);
    }];
...
-(void)jueSeQieHuanBtChick:(UIButton *)sender
{
    SwitchRolesViewController *vc = [[SwitchRolesViewController alloc]init];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.xiaoXilabel.text = [NSString stringWithFormat:@"%ld",(unsigned long)[UserInfo shareInstance].userDingDanArray.count];
}
...
-(void)postHuoQuModele
{
    ...
    [NetWorkManager requestWithParameters:mDict withUrl:@"data_center/privateinfo/staff_detail" viewController:self withRedictLogin:YES isShowLoading:NO success:^(id responseObject) {
        ...
        weakSelf.jueSeQieHuanYouLabel.text = [NSString stringWithFormat:@"%@",KISDictionaryHaveKey(adData, @"role_name")];

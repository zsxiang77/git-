...
-(void)danGeView
{
    ...
    UIImageView *baimView = [[UIImageView alloc]initWithFrame:CGRectMake((kWindowW-360/4)/2, kNavBarHeight+150, 360/4, 310/4)];
    baimView.image = DJImageNamed(@"web_jiaZaiZHong");
    [self.view addSubview:baimView];
    ...
    m_webView[0].scrollView.backgroundColor = [UIColor clearColor];
...
- (void)buildMainViewWitharray:(NSArray *)array
{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        UIImageView *baimView = [[UIImageView alloc]initWithFrame:CGRectMake((kWindowW-360/4)/2, kNavBarHeight+50, 360/4, 310/4)];
        baimView.image = DJImageNamed(@"web_jiaZaiZHong");
        [maiTabelView[i] addSubview:baimView];
        ...
        m_webView[i].scrollView.backgroundColor = [UIColor clearColor];
        if (i == 0) {
            m_webView[0].scrollView.mj_header = [MJChiBaoZiHeader headerWithRefreshingTarget:self refreshingAction:@selector(loadNewData0)];
        }else{

...
-(void)quanXuanBtChick:(UIButton *)sender
{
    ...
    NSMutableArray *puanduanArray = [[NSMutableArray alloc]init];
    for (int i = 0; i<self.zongDataArray.count; i++) {
        ...
        for (int j = 0; j<array2.count; j++) {
            ...
            if (modelStaff.shiFouXuanZhong == YES) {
                [puanduanArray addObject:modelStaff];
            }
        }
    }
    ...
    if ((puanduanArray.count+array.count)>=4) {
        [self showMessageWindowWithTitle:@"最多选择4人" point:self.view.center delay:1];
        return;
    }

...
@interface NewMaterialViewController ()<UITableViewDelegate,UITableViewDataSource,DuplexTableDelegate>
{
...
@implementation NewMaterialViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.zhuImageView  sd_setImageWithURL:[NSURL URLWithString:mode.images] placeholderImage:DJImageNamed(@"")];
    ...
    if (mode.current_count.length<=0) {
        cell.kuCunLabel.text = @"0.00";
    }else{
    ...
    if (mode.xuanZhong == YES) {
        cell.xuanZhonImageView.image = DJImageNamed(@"cell_select");
    }else{
        cell.xuanZhonImageView.image = DJImageNamed(@"cell_noselect");
    }

...
- (void)getRecogResult{
    ...
    if (![allResult isEqualToString:@""]) {
        ...
        if (self.suerViewColler) {
            //跳转到结果展示页面
            FillInformationViewController *vc = (FillInformationViewController *)self.suerViewColler;
            vc.chuanZhiDict = chuanZhiDict;
            [self dismissViewControllerAnimated:YES completion:nil];
        }else
        {
            //跳转到结果展示页面
            FillInformationViewController *rvc = [[FillInformationViewController alloc] init];
            rvc.zhuModel = self.zhuModel;
            NSLog(@"allresult = %@", allResult);
            rvc.cropImagepath = _cropImagepath;
            rvc.chuanZhiDict = chuanZhiDict;
            rvc.zuiZhongModel = self.zuiZhongModel;

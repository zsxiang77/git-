...
- (void)loadFromProperty
{
    ...
    self.isAnimated = YES;
    self.strokeColor = kZhuTiColor;
    self.inStrokeColor = [UIColor redColor];
    self.spotColor = [UIColor redColor];
    self.fillColor = [UIColor colorWithRed:0.000 green:0.000 blue:1.000 alpha:0];
...
- (void)createLabels
{
    ...
    CGFloat radius = self.frame.size.width/2 - LABEL_WIDTH/2-30;
    CGPoint center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);

    for (int i = 0;i < self.nameArr.count; i ++) {
        if(i==0){
            radius = self.frame.size.width/2 - LABEL_WIDTH/2-40;
        }else{
           radius = self.frame.size.width/2 - LABEL_WIDTH/2-20;
        }
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat angle = 360.0/sideCount;
    CGFloat radius = self.frame.size.width/2 - LABEL_WIDTH-10;
    CGPoint center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
    ...
    if (sideCount > 0) {
        for (int i = 0; i < sideCount + 1; i ++) {
            CGFloat progress = [self.processArr[i%sideCount] floatValue]/100;
        ...
        CGPoint peakCenter = [self calcCircleCoordinateWithCenter:CGPointMake(center.x, center.y) andWithAngle:90 andWithRadius:self.frame.size.width/2 - LABEL_WIDTH];
        UILabel *nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(peakCenter.x- LABEL_WIDTH/2, 40, LABEL_WIDTH, LABEL_WIDTH)];
        nameLabel.textColor = kRGBColor(74,144,226);
        nameLabel.font = [UIFont systemFontOfSize:14];
        nameLabel.textAlignment = NSTextAlignmentCenter;
        nameLabel.text = @"能力值";
        [self addSubview:nameLabel];
        
        UILabel* nengLizhiLabel = [[UILabel alloc] initWithFrame:CGRectMake(peakCenter.x- LABEL_WIDTH/2,110, LABEL_WIDTH, LABEL_WIDTH)];
        nengLizhiLabel.textColor = kRGBColor(74,144,226);
        nengLizhiLabel.font = [UIFont systemFontOfSize:14];
        nengLizhiLabel.textAlignment = NSTextAlignmentCenter;
        nengLizhiLabel.text = self.nengLizhi;
        [self addSubview:nengLizhiLabel];

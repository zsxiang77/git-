    ...
    for (NSInteger i = 0; i<_dataArr.count; i++) {
        ...
        for (id obj in _dataArr[i]) {
            ...
            if ([obj isKindOfClass:[NSArray class]]) {
                if (model.maxCount<=[(NSArray *)obj count]) {
                    model.maxCount = [(NSArray *)obj count];
                }
            }
        }
    ...
    for (NSInteger i = 0; i<_dataModelArr.count; i++) {
        ...
        for (NSInteger j = 0; j< model.dataArr.count; j++) {
            ...
            if ([rowItems isKindOfClass:[NSArray class]]) {
                ...
                CGFloat perItemsHeightByMaxCount = model.maxCount * _minHeightItems / [(NSArray *)rowItems count];
                /*       具体某一列有多个元素时       */
                for (NSInteger n = 0; n<[(NSArray *)rowItems count]; n++) {
    ...
    for (NSArray * itemsArr in _dataArr) {
        ...
        for (id obj in itemsArr) {
            ...
            if ([obj isKindOfClass:[NSArray class]]) {
                ...
                if (nowCount<=[(NSArray *)obj count]) {
                    nowCount = [(NSArray *)obj count];
                }

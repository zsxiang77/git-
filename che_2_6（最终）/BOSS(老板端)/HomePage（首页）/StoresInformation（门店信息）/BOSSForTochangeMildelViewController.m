//
//  BOSSForTochangeMildelViewController.m
//  cheDianZhang
//
...
@interface BOSSForTochangeMildelViewController ()
<UITextFieldDelegate>
@property(nonatomic,strong)UITextField*mainTextFile;
@end

@implementation BOSSForTochangeMildelViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTopViewWithTitle:@"门店电话" withBackButton:YES];
    self.mainTextFile=[[UITextField alloc]initWithFrame:CGRectMake(10,kBOSSNavBarHeight , kWindowW-20, 61)];
    self.mainTextFile.placeholder=@"请输入门店电话";
    self.mainTextFile.delegate=self;
    self.mainTextFile.text = KISDictionaryHaveKey(self.chaunzhiMdisn,@"phone");
...
-(void)buttonRight:(UIButton*)seader
{
    if(self.mainTextFile.text.length<=0){
        [self showMessageWithContent:@"请输入门店电话" point:self.view.center afterDelay:1];
        return;
    }
    kWeakSelf(weakSelf)
    NSMutableDictionary*mDict=[NSMutableDictionary dictionaryWithCapacity:10];
    [mDict setObject:self.mainTextFile.text forKey:@"phone"];

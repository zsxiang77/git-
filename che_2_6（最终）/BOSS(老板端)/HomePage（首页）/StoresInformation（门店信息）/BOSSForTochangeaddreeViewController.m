//
//  BOSSForTochangeaddreeViewController.m
//  cheDianZhang
//
...
@interface BOSSForTochangeaddreeViewController ()
<UITextFieldDelegate>
@property(nonatomic,strong)UITextField*mainTextFile;
@end

@implementation BOSSForTochangeaddreeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTopViewWithTitle:@"门店地址" withBackButton:YES];
...
-(void)buttonRight:(UIButton*)seader
{
    if(self.mainTextFile.text.length<=0){
        [self showMessageWithContent:@"请输入您的门店地址" point:self.view.center afterDelay:1];
        return;
    }
    kWeakSelf(weakSelf)
    NSMutableDictionary*mDict=[NSMutableDictionary dictionaryWithCapacity:10];
    [mDict setObject:self.mainTextFile.text forKey:@"address"];

...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        im2 = [[UIImageView alloc]initWithImage:DJImageNamed(@"boss_Found_dianZan")];
        ...
        [btn addTarget:self action:@selector(buttionZan:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [m_dateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.mas_equalTo(-10);
            make.centerY.mas_equalTo(m_zanLabel);
        }];
...
-(void)refleshData:(FoundModel *)dict
{
    ...
    if ([dict.is_praise boolValue] == YES) {
        im2.image = DJImageNamed(@"boss_Found_dianZan_select");
        m_zanLabel.textColor = kRGBColor(208, 2, 17);
    }else{
        im2.image = DJImageNamed(@"boss_Found_dianZan");
        m_zanLabel.textColor = kRGBColor(155, 155, 155);
    }
}
-(void)buttionZan:(UIButton*)seader
{
    self.changePartst(self.zhuModel);

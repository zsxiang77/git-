...
//    path = @"http://inflexion.icarzoo.com/api/user/ucenter/store_info";
...
//        NPrintLog(@"qweasd:%@",str);
        ...
        [manager POST:path parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
            ...
            NPrintLog(@"\nAPI:%@\n参数：%@\n返回：%@", url, [[NSString alloc] initWithData:cookieData encoding:NSUTF8StringEncoding], parserDict);
            
//            NSData *cookieData2 = [NSJSONSerialization dataWithJSONObject:parserDict
//                                                                 options:NSJSONWritingPrettyPrinted
//                                                                   error:nil];
//            NPrintLog(@"\n返回：%@", [[NSString alloc] initWithData:cookieData2 encoding:NSUTF8StringEncoding]);

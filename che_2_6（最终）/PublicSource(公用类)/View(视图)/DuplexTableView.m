...
- (void)buildMainViewWithTabel:(NSArray*)array head:(UIView*)hView visibleHeight:(float)visibalH isHaveDownRefresh:(BOOL)isHaveDown
{
    ...
    if (@available(iOS 11.0, *)) {
        _fatherScroll.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
    }
    ...
    for (int i = 0; i < [array count]; i++) {
        if (@available(iOS 11.0, *)) {
            UITableView *taView = tableArray[i];
            taView.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
        }

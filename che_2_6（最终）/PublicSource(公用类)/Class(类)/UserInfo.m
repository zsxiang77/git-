...
- (id)init
{
    ...
    if (self) {
        ...
        self.userPositions = [[NSArray alloc]init];
        [self buildUserName];
...
+ (void)saveUserName
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[UserInfo shareInstance].userPositions forKey:userPositions];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (void)buildUserName
{
    if (([[NSUserDefaults standardUserDefaults] objectForKey:kUserToken] != nil)&&([[NSUserDefaults standardUserDefaults] objectForKey:kIsExplod] != nil)) {
        ...
        self.userPositions = [[NSUserDefaults standardUserDefaults] objectForKey:userPositions];
        self.isLogined = YES;
    }else{
        [self cleanUserInfor];
    }
...
+ (void)saveuseruserDingDanArray:(NSDictionary *)dict
{
    ...
    for (int i = 0; i<[UserInfo shareInstance].userDingDanArray.count; i++) {
        if ([KISDictionaryHaveKey([UserInfo shareInstance].userDingDanArray[i], @"ordercode") isEqualToString:KISDictionaryHaveKey(dict, @"ordercode")]) {
            [xiTongArray removeObject:[UserInfo shareInstance].userDingDanArray[i]];
        }
    }
...
+ (void)dicToJson:(NSDictionary *)dic{
    if ([NSJSONSerialization isValidJSONObject:(NSDictionary
        ...
        NSError
        *error1;
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:(NSDictionary *)dic options:NSJSONWritingPrettyPrinted error
                                                                  :&error1];
